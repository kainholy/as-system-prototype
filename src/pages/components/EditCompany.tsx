...
function EditCompany({ setEditOpen, companyId }: EditCompanyProps) {
  ...
  const handleDelete = async () => {
    if (editedCompany) {
      try {
        await axios.delete(`http://localhost:4000/companies/${editedCompany.id}`);
        alert("会社情報が削除されました。");
        editCloseFunc();
        window.location.reload(); // ページをリロード
      } catch (error) {
  ...
  return (
    <Box position='absolute' top='52px' left='0' backgroundColor='blackAlpha.500' w='100%' h='100vh' zIndex='5'>
      ...
      >
        ...
        <FormControl isRequired>
          <FormLabel fontSize="sm" color="gray.800">住所</FormLabel>
          <Input
            type="text"
            name="address"
        ...
        <Flex gap="20px" justifyContent='right'>
          <Button onClick={handleUpdate} mt={4} pl={12} pr={12} colorScheme="blue">
            更新
          </Button>
          <Button onClick={handleDelete} mt={4} pl={12} pr={12} colorScheme="red">
            削除
          </Button>
        </Flex>
      </Flex>
